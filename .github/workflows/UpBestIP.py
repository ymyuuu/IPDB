...
# 子域名与对应的 IP 列表 URL 配置
# key 可以带后缀，如 "v4@10" 表示获取前 10 个 IP，"v6@all" 表示获取所有 IP
subdomain_configs = {
    "bestcf": {
        "v4": "https://raw.githubusercontent.com/ymyuuu/IPDB/refs/heads/main/BestCF/bestcfv4.txt",
        "v6": "https://raw.githubusercontent.com/ymyuuu/IPDB/refs/heads/main/BestCF/bestcfv6.txt"
...
# 获取指定 URL 的 IP 列表，可根据 count 参数决定返回数量
# 如果 count 为 "all"，则返回全部 IP，否则返回前 count 行 IP（默认值为2）
def fetch_ip_list(url: str, count=2) -> list:
    response = requests.get(url)  # 发送 GET 请求获取数据
    response.raise_for_status()   # 检查响应状态，若请求失败则抛出异常
    ip_lines = response.text.strip().split('\n')
    if count == "all":
...
def main():
    try:
        ...
        for idx, token in enumerate(api_tokens, start=1):
            ...
            # 遍历所有子域名配置
            for subdomain, version_urls in subdomain_configs.items():
                # 针对每个版本（如 v4、v6 或带后缀的 v4@10 等）分别处理
                for key, url in version_urls.items():
                    # 解析 key，如 "v4@10" 解析出 version 为 "v4"，count 为 "10"
                    if "@" in key:
                        version_key, count = key.split("@", 1)
                    else:
                        version_key = key
                        count = 2  # 默认取前两个 IP
                    dns_type = dns_record_map.get(version_key)
                    if not dns_type:
                        continue
                    # 获取 IP 列表，根据 count 决定返回 IP 的数量
                    ips = fetch_ip_list(url, count)
                    # 删除旧的 DNS 记录
                    update_dns_record(token, zone_id, subdomain, domain, dns_type, "delete")
                    # 添加新的 DNS 记录
                    if ips:
