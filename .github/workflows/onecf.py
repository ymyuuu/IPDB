...
def update_dns_records(zone_id, name, dns_domains, headers, excluded_networks, protected_ip):
    unique_ips = set()
    ...
    for dns_domain in dns_domains:
        ...
        for record in data.get("result", []):
            if name == "@" or re.search(name, record.get("name", "")):
                if record.get("content", "") == protected_ip:
                    continue  # Skip deleting A records with the protected IP
                delete_dns_record(zone_id, record.get("id", ""), headers)

    filtered_ips = list(set(ip for ip in unique_ips if not any(ip_address(ip) in ip_network(net) for net in excluded_networks) and ip != protected_ip))
...
if __name__ == "__main__":
    ...
    protected_ip = "76.76.21.21"

    headers = {"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"}

    update_dns_records(zone_id, name, dns_domains, headers, excluded_networks, protected_ip)
